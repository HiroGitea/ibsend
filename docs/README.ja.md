...
QDR 環境でのテストでは **3.47 GB/s** に達しました。テスト環境と転送時間の測定方法は
...
[コマンド](#コマンド) · [コンテナ](#コンテナと-kubernetes) ·
[パフォーマンス](#パフォーマンス) · [制限事項](#制限事項)
...
- **複数ファイルの転送とピア検出**：複数のパスを一度に送信し、IPoIB サブネット上で
  受信側を検出できます。GUI でのドラッグ＆ドロップにも対応しています。
- **コンテナと自動化**：Docker や Kubernetes で実行でき、JSON 出力と固定の終了コードに
  よってスクリプトに組み込めます。
...
ソースからのビルドには、最近の安定版 Rust ツールチェーン、C コンパイラ、
...
`--bind <RDMA-IPv4-address>` で待ち受けアドレスを指定し、送信側からそのアドレスを直接指定してください。
...
| `--port <N>` | `18515` | `rdma_cm` のポート。 |
| `--ready-file <PATH>` | なし | 受信の準備ができたときに作成するファイル。 |
| `--json` | オフ | JSON イベントを標準出力に 1 行ずつ出力します。 |

サイズには `K`、`M`、`G` の接尾辞を使用でき、1024 の累乗で解釈されます。
例えば `--pool 2G` は 2 GiB のプールを要求します。送信側の `--slabs <N>` は
パイプラインの深さを指定し、既定値は 16 です。`--prefix <DIR>` を指定すると、
受信側の保存先の下にあるサブディレクトリにファイルを配置します。
`discover --timeout <MS>` はアドレスごとの解決タイムアウトを指定し、既定値は
300 ミリ秒です。`send` と `discover` も `--port` と `--json` に対応しています。
JSON イベントと終了コードは[スクリプト用インターフェイス](containers.md#scripting-interface)（英語）
を参照してください。引数なしで `ibsend` を実行すると使い方を表示します。
...
リポジトリのルートでコンテナイメージをビルドします。

```sh
docker build -t ibsend .
```

コンテナには、ホストの IPoIB ネットワーク、RDMA デバイス、`IPC_LOCK` ケーパビリティが
必要です。`inbox` ディレクトリは UID 10001 から書き込める必要があります。
...
受信側のバッファプールは、コンテナのメモリ制限に合わせてサイズが決まります。
Kubernetes 向けには、Helm チャート、YAML 形式のマニフェスト、ワークロード例、
`kubectl ibsend` プラグインを用意しています。詳しくは
[コンテナと Kubernetes](containers.md)（英語）を参照してください。
...
**送信側の測定時間はデータ送信のみ、受信側の測定時間はファイル書き込みまでを含みます。** メモリバッファにより、
